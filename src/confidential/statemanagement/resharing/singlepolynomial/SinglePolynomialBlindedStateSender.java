...
import confidential.statemanagement.resharing.RecoveryContribution;
...
public class SinglePolynomialBlindedStateSender extends Thread {
    ...
    private final PolynomialPoint blindingShare;
    ...
    public SinglePolynomialBlindedStateSender(ServerViewController svController, BigInteger field,
                                              int stateReceiverPort, int[] receivers,
                                              DefaultApplicationState state, PolynomialPoint blindingShare,
    ...
    public void run() {
        ...
        long t1, t2;
        t1 = System.nanoTime();
        BlindedApplicationState[] blindedStates = createBlindedState(blindingShare, state);
        t2 = System.nanoTime();
        ...
        try (ByteArrayOutputStream bosCommonState = new ByteArrayOutputStream();
            ...
            for (int i = 0; i < receivers.length; i++) {
                try (ByteArrayOutputStream bosCommitments = new ByteArrayOutputStream();
                    ...
                    vss.Utils.writeCommitment(blindingShare.getCommitments(receivers[i]), outCommitments);//R commitments
                    VerifiableShare[] shares = recoveryContribution.getShares();
                    for (VerifiableShare share : shares) {
                        vss.Utils.writeCommitment(share.getCommitments(), outCommitments);
                        blindedShares.offer(share.getShare());
                    }
    ...
    private RecoveryContribution[] processShares(LinkedList<VerifiableShare> sharesToSend,
                                                 PolynomialPoint blindingShare) throws InterruptedException, IOException {
        ...
        for (int i = 0; i < receivers.length; i++) {
            ...
            recoveryContributions[i] = new RecoveryContribution(verifiableShares, null);
        }
